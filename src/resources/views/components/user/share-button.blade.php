{{-- 商品共有ボタンコンポーネント --}}
@props([
    'title' => '',
    'url' => null,
])

@php
    $shareUrl = $url ?? request()->url();
    $encodedUrl = urlencode($shareUrl);
    $encodedTitle = urlencode($title);
    $componentId = 'share-' . uniqid();
@endphp

<div class="relative" x-data="{ open: false }">
    {{-- シェアボタン --}}
    <button
        @click="open = !open"
        class="p-4 rounded-2xl border border-gray-200 hover:bg-gray-50 transition-all group"
        title="共有"
    >
        <i data-lucide="share-2" class="w-6 h-6 text-gray-400 group-hover:text-indigo-600 transition-colors"></i>
    </button>

    {{-- シェアメニュー --}}
    <div
        x-show="open"
        @click.away="open = false"
        x-transition:enter="transition ease-out duration-200"
        x-transition:enter-start="opacity-0 translate-y-2"
        x-transition:enter-end="opacity-100 translate-y-0"
        x-transition:leave="transition ease-in duration-150"
        x-transition:leave-start="opacity-100 translate-y-0"
        x-transition:leave-end="opacity-0 translate-y-2"
        class="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50"
        style="display: none;"
    >
        <div class="px-4 py-2 border-b border-gray-100">
            <p class="text-xs font-bold text-gray-500 uppercase tracking-wider">この商品を共有</p>
        </div>

        <div class="py-2">
            {{-- X (Twitter) --}}
            <a
                href="https://twitter.com/intent/tweet?text={{ $encodedTitle }}&url={{ $encodedUrl }}"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors"
            >
                <div class="w-10 h-10 bg-black rounded-xl flex items-center justify-center">
                    <svg class="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                    </svg>
                </div>
                <span class="font-medium text-gray-700">X (Twitter)</span>
            </a>

            {{-- Facebook --}}
            <a
                href="https://www.facebook.com/sharer/sharer.php?u={{ $encodedUrl }}"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors"
            >
                <div class="w-10 h-10 bg-[#1877F2] rounded-xl flex items-center justify-center">
                    <svg class="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                </div>
                <span class="font-medium text-gray-700">Facebook</span>
            </a>

            {{-- LINE --}}
            <a
                href="https://social-plugins.line.me/lineit/share?url={{ $encodedUrl }}"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors"
            >
                <div class="w-10 h-10 bg-[#00B900] rounded-xl flex items-center justify-center">
                    <svg class="w-6 h-6 text-white" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M19.365 9.863c.349 0 .63.285.63.631 0 .345-.281.63-.63.63H17.61v1.125h1.755c.349 0 .63.283.63.63 0 .344-.281.629-.63.629h-2.386c-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63h2.386c.349 0 .63.285.63.63 0 .349-.281.63-.63.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.629-.631.629-.346 0-.626-.285-.626-.629V8.108c0-.27.173-.51.43-.595.06-.023.136-.033.194-.033.195 0 .375.104.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.629-.631.629-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63.349 0 .631.285.631.63v4.771zm-2.466.629H4.917c-.345 0-.63-.285-.63-.629V8.108c0-.345.285-.63.63-.63.348 0 .63.285.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .344-.282.629-.629.629M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314"/>
                    </svg>
                </div>
                <span class="font-medium text-gray-700">LINE</span>
            </a>
        </div>

        <div class="border-t border-gray-100 py-2">
            {{-- URLをコピー --}}
            <button
                @click="$refs.copyText.textContent = 'コピーしました！'; $refs.copyText.classList.add('text-green-600'); navigator.clipboard.writeText('{{ $shareUrl }}'); setTimeout(() => { $refs.copyText.textContent = 'URLをコピー'; $refs.copyText.classList.remove('text-green-600'); }, 2000)"
                class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors w-full text-left"
            >
                <div class="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center">
                    <i data-lucide="link" class="w-5 h-5 text-gray-600"></i>
                </div>
                <span class="font-medium text-gray-700" x-ref="copyText">URLをコピー</span>
            </button>
        </div>
    </div>
</div>
